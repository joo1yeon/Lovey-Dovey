            ...
            public void onClick(View v) {
                if (edtID.getText().toString().getBytes().length > 0) {
                    ...
                    res.enqueue(new Callback<ResponseConn>() {
                        @Override
                        public void onResponse(Call<ResponseConn> call, Response<ResponseConn> r) {
                            if (r.isSuccessful()) {
                                if(r.body().getSuccess()){
                                    Log.d("NAMEE", edtID.getText().toString());
                                    AlertDialog.Builder dlg = new AlertDialog.Builder(CoupleConnect.this);
                                    dlg.setMessage(edtID.getText().toString() + "과 연결하시겠습니까?")
                                                    ...
                                                    res.enqueue(new Callback<responseConnect>() {
                                                        ...
                                                        public void onResponse(Call<responseConnect> call, Response<responseConnect> response) {
                                                            if (response.body().getConnect()) {
                                                                ...
                                                                Toast.makeText(CoupleConnect.this, "상대방이 다른 사용자와 연결되어있습니다.", Toast.LENGTH_SHORT).show();
                                                        }
                                ...
                                }else Toast.makeText(CoupleConnect.this, "입력하신 정보와 일치하는 상대방의 정보가 없습니다.", Toast.LENGTH_SHORT).show();
                            }
                        }

                        @Override
                        public void onFailure(Call<ResponseConn> call, Throwable t) {
                            Toast.makeText(CoupleConnect.this, "입력하신 정보와 일치하는 상대방의 정보가 없습니다.", Toast.LENGTH_SHORT).show();
                        }
                    });

                } else
                    Toast.makeText(CoupleConnect.this, "상대방의 아이디를 입력해주세요.", Toast.LENGTH_SHORT).show();
            }
        });
    }
